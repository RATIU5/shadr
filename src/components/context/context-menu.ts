...
export function initializeContextComponent(
  element: ICanvas,
  initialItems: ContextItem[]
) {
  let contextMenuInstance = contextMenu(element, initialItems);
  return {
    updateItems: (newItems: ContextItem[]) =>
      contextMenuInstance.update(newItems),
      ...
      li.addEventListener("mouseenter", () => {
        ...
        setTimeout(() => {
          ...
          if (submenuRect.bottom > viewportHeight) {
            submenu.style.top = `-${
              submenuRect.height - li.offsetHeight - 3
            }px`;
          }
        }, 0);
      });
