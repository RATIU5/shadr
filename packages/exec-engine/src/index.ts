...
export const markDirty = (graph: Graph, state: DirtyState, nodeId: NodeId): DirtyState => {
...
export const clearDirty = (state: DirtyState, nodeIds: Iterable<NodeId>): DirtyState => {
	const dirty = new Set(state.dirty);
...
export const isDirty = (state: DirtyState, nodeId: NodeId): boolean => state.dirty.has(nodeId);
